...
psImproveApp = function(sum.df, err.df=NULL, hint.df=NULL, log.df = rbind(err.df, hint.df), sol.dir = getwd()) {
  ...
  customEventHandler("tabClick",css.locator = "#df_tab tr", event="click", function(id,..., app) {
    ...
    if (FALSE) {
      err.txt = combine_errors(filter(err.df, chunk==cd$chunk, ps.name==cd$ps.name))
      
      txt = merge.lines(err.txt, collapse="\n**************************\n")
      
      txt = paste0(cd$ps.name, " ", cd$chunk.name, " ", length(err.txt), " wrong solutions:\n",txt)
    }
    ...
    try(open.sol.rmd(cd$ps.name, cd$chunk.name, sol.dir=sol.dir))
    #cat("\nClick!!!")
  })
  app
}

make.chunk.log.txt = function(log.df, ps.name, chunk) {
  restore.point("make.chunk.log.txt")
  ...
  all.txt = lapply(li, function(udf) {
    ...
    rows = udf$type == "check_chunk"
    txt[rows] = udf$code[rows]
...
open.sol.rmd = function(ps.name,chunk.name=NULL, sol.postfix = "_sol.Rmd", sol.dir=getwd()) {
  restore.point("open.sol.rmd")
  library(rstudioapi)
  
  file = file.path(sol.dir, paste0(ps.name,sol.postfix))
...
analyse.sub = function(base.dir=getwd(), prefix="",postfix=".zip", rps.dir=base.dir,  unpack.big = !dir.exists(file.path(base.dir,"sub"))) {
